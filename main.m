...
if ANIMATION
    ...
    txt2 = 'Time:';
    axis([0 900 0 700])
    xlim = get(gca,'xlim')-200;
    ylim = get(gca,'ylim')+20;
    text(xlim(2),ylim(2),txt2);
end
...
while dt < 100000
    ...
    if ANIMATION
        txt = num2str(time);
        htext=text(xlim(2)+85,ylim(2),txt);
        M(dt) = getframe(gcf);
        pause(0.001);
        delete(htext)
        for i=1:totalVehicles
...
if ANIMATIONPP  
    ...
    for  t = 0:timeStep:TMax%
        
        frame = int8((1/timeStep)*t+1);
        
        for i = 1:totalVehicles
            if t < x(i).Time(1) || t > x(i).Time(end)
                continue
            end
            if t >= x(i).Time(1) && index(i) < 1
               index(i) = 1;
            end
            PathNumber = path(i);
            if PathNumber == 1 %&& finish == 0
                % path #1
                xx(i) = 2*roadLength+5*mergeLength/4;
                yy(i) = 2*roadLength+mergeLength- x(i).Position(index(i));
                hh(i) = plot(xx(i),yy(i),'.m');
                HD(i) = gamma + x(i).Velocity(index(i))*phi;
                c(i) = circle(xx(i),yy(i),HD(i));
                hold on;
            elseif PathNumber == 2
                % path #2
                if x(i).Position(index(i))<(roadLength+mergeLength/4)
                    xx(i) = roadLength+3*mergeLength/4;
                    yy(i) = x(i).Position(index(i));
                else
                    xx(i) = x(i).Position(index(i))+(mergeLength/2);
                    yy(i) = roadLength+mergeLength/4;
                end
                hh(i) = plot(xx(i),yy(i),'.k');
                HD(i) = gamma + x(i).Velocity(index(i))*phi;
            ...
            elseif PathNumber == 3
                ...
                hold on
            elseif PathNumber == 4
                if x(i).Position(index(i))< roadLength+3*mergeLength/4
                    xx(i) = 2*roadLength+7*mergeLength/4;
                    yy(i) = x(i).Position(index(i));
                elseif x(i).Position(index(i))< 2*roadLength+9*mergeLength/4
                    xx(i) = -x(i).Position(index(i))+(3*roadLength+5*mergeLength/2);
                    yy(i) = roadLength+3*mergeLength/4;
                else
                    xx(i) =  roadLength+mergeLength/4;
                    yy(i) = -x(i).Position(index(i))+(3*roadLength+3*mergeLength);
                end
                hh(i) = plot(xx(i),yy(i),'.b');
                HD(i) = gamma + x(i).Velocity(index(i))*phi;
                c(i) = circle(xx(i),yy(i),HD(i));
